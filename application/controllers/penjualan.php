...
//    GET DATA
    ...
    function get_detail_barang(){
        ...
        $data=array(
            'detail_barang'=>$this->model_app->getSelectedData('tbl_mobil',$id)->result(),
        );
...
//    INSERT DATA
    ...
    function simpan_penjualan(){
        ...
        foreach($this->cart->contents() as $items){
            ...
            $update['stok'] = $this->model_app->getKurangStok($kd_mobil,$qty);
            $key['kd_mobil'] = $kd_mobil;
            $this->model_app->updateData("tbl_mobil",$update,$key);
        }
        $this->session->unset_userdata('limit_add_cart');
        $this->cart->destroy();
        redirect('penjualan');
...
//    DELETE
    function hapus_barang(){
        ...
        else if($kode[0]=="edit")
        {
            ...
            $key_barang['kd_mobil'] = $hps['kd_mobil'];
            $d_u['stok'] = $kode[4]+$kode[5];
            $this->model_app->updateData("tbl_mobil",$d_u,$key_barang);
        }
    ...
    function hapus(){
        ...
        foreach($q->result() as $d){
            $d_u['stok'] = $this->model_app->getTambahStok($d->kd_mobil,$d->qty);
            $key['kd_mobil'] = $d->kd_mobil;
            $this->model_app->updateData("tbl_mobil",$d_u,$key);
        }
