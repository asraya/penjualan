...
class Model_app extends CI_Model{
    ...
    public function getTambahStok($kd_mobil,$tambah)
    {
        $q = $this->db->query("select stok from tbl_mobil where kd_mobil='".$kd_mobil."'");
        $stok = "";
    ...
    public function getKurangStok($kd_mobil,$kurangi)
    {
        $q = $this->db->query("select stok from tbl_mobil where kd_mobil='".$kd_mobil."'");
        $stok = "";
        foreach($q->result() as $d)
    ...
    public function getKembalikanStok($kd_mobil)
    {
        $q = $this->db->query("select stok from tbl_mobil where kd_mobil='".$kd_mobil."'");
        $stok = "";
        foreach($q->result() as $d)
    ...
    function getBarangJual(){
        return $this->db->query ("SELECT * from tbl_mobil where stok > 0")->result();
    }
    ...
    function getBarangPenjualan($id){
        return $this->db->query("
                select a.kd_mobil,a.qty,b.nm_mobil,b.harga
                from tbl_penjualan_detail a
                left join tbl_mobil b on a.kd_mobil=b.kd_mobil
                where a.kd_penjualan = '$id'")->result();
    }
