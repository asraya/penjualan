...
<div id="modalAddPenjualanBarang" class="modal hide fade">
    <div class="modal-header">
        ...
        <h3 id="myModalLabel">Pemilihan Data Mobil</h3>
    </div>
    <form id="frm" name="frm"  method="post" action="<?php echo site_url('penjualan/tambah_penjualan_to_cart')?>">
        <div class="modal-body" style="min-height: 200px">
            <div class="control-group">
                <label class="control-label">Daftar Mobil</label>
                <div class="controls">
                    <select id="kd_mobil" class="chzn-select" name="kd_mobil" data-placeholder="Pilih Barang">
                        <option value=""></option>
                        <?php
                        if(isset($data_barang)){
                            foreach($data_barang as $row){
                                ?>
                                <option value="<?php echo $row->kd_mobil?>"><?php echo $row->nm_mobil?></option>
                            <?php
                            }
                        }
                        ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#kd_mobil").change(function(){
            var kd_mobil = $("#kd_mobil").val();
            $.ajax({
                type: "POST",
                url : "<?php echo base_url('penjualan/get_detail_barang'); ?>",
                data: "kd_mobil="+kd_mobil,
                cache:false,
